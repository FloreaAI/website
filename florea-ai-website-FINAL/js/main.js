...
function openModal(memberId) {
  ...
  document.getElementById('modal-education').innerHTML = member.education.replace(/\\n/g, '<br>');
...
function loadPublications(filter = 'all') {
  ...
  filteredPubs.forEach(pub => {
    ...
    pubCard.innerHTML = `
      <h3>${pub.title}</h3>
      <p class="publication-meta">${pub.authors} (${pub.year}) \n ${pub.journal}</p>
      <p>${pub.abstract}</p>
...
function filterPublications(filter, event) {
...
function loadBlogPosts(filter = 'all') {
  ...
  if (typeof blogPosts === 'undefined' || !blogPosts) {
    ...
    blogGrid.innerHTML = '<p style="text-align: center; color: red;">Error: Blog data not loaded. Please check data.js file.</p>';
    return;
  }
  blogGrid.innerHTML = '';
...
function loadLatestUpdates() {
  ...
  if (typeof publications === 'undefined' || !publications || typeof blogPosts === 'undefined' || !blogPosts) {
    console.error('Latest updates data not available!');
  }
